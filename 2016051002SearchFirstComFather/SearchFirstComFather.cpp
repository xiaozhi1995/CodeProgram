...
protected:
	...
	BinaryTreeNode* _GetFather(BinaryTreeNode* root, BinaryTreeNode* node1, BinaryTreeNode* node2)
	{
		if (root == NULL)
			return NULL;
		if (root == node1 || root == node2)
			return root;
		BinaryTreeNode* left = _GetFather(root->m_pLeft, node1, node2);
		BinaryTreeNode* right = _GetFather(root->m_pRight, node1, node2);
		if (left&&right)
		{
			return root;
		}
...
public:
	...
	BinaryTreeNode* SearchFirstComFather(BinaryTreeNode* childNode1, BinaryTreeNode* childNode2)
	{
		return _GetFather(_root, childNode1, childNode2);
	}
};
void Test1()
{
